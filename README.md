...
Задание 3:
   - Сначала при инициализации мы составляем список документов в которых встречается каждый токен и каждая лемма в виде json, например:
   "bin": [
    1,
    141,
    147
  ]
  - После этого идет работа с поисковым запросом -
     * 1)токенизация запроса с помощью regex выражения
     * 2)операции имют определенный порядок в котором выполняются для каждой группы - AND> OR> NOT(или "-")
     * 3)для каждой группы идет расчет результата запроса, после идет расчет общего результата сначала OR затем NOT
  - все шаги выводятся в консоли, после завершения выводится список из страниц и ссылок на них

Задание 4
   - Собирается статистика по всем скачанным страницам
   - Считается TF = количество вхождений термина / общее количество терминов в документе
   - Считается IDF = log(всего документов / документы с термином)
      * TF-IDF = TF*IDF
   -Для каждой страницы создаются два файла:
       * tfidf_tokens.txt — TF-IDF для терминов
       * tfidf_lemmas.txt — TF-IDF для лемматизированных форм
